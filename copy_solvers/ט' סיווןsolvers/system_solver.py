    ...
    def solve(self):
        ...
        for s in sols:
            try:
                ...
                self._add_step(
                    f"נציב $${sp.latex(other_var)} = {sp.latex(val_other)}$$ בביטוי $${sp.latex(iso_var)} = {sp.latex(MathUtils.clean_floats(iso_expr))}$$ונקבל:")
        ...
        # עדכון בניית המחרוזת כך שתציג x = ... ו- y = ... עבור כל זוג פתרונות
        res_final = " \\quad , \\quad ".join([f"x = {sp.latex(p[0])} \\quad , \\quad y = {sp.latex(p[1])}" for p in final_pairs])
...
def solve_system_steps(equations_text):
    """פונקציית הממשק ל-Flask"""
    try:
