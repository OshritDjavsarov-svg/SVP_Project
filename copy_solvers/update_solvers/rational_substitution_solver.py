...
#
#     def solve(self):
#         self.steps.append("🚀 זיהיתי מערכת משוואות עם נעלמים במכנה. נשתמש בשיטת הצבת משתני עזר.")
#
#         # 1. הגדרת משתני העזר u ו-v
#         self.steps.append("### שלב 1: הגדרת משתני עזר")
#         self.steps.append("נזהה ביטויים חוזרים ונגדיר: $u = \\frac{1}{x}$ ו-$v = \\frac{1}{y}$")
#
#         u, v = sp.symbols('u v')
#
#         # פונקציית עזר להמרת ביטויים עם x,y לביטויים עם u,v בצורה מתמטית בטוחה
#         def transform_string(eq_s):
#             # החלפה חכמה של תבניות כמו 1/x, 2/x, 5/y ל-u ו-v
#             s = eq_s.replace(' ', '')
#             s = re.sub(r'(\d+)/x', r'\1*u', s)  # הופך למשל 2/x ל-2*u
#             s = re.sub(r'1/x', r'u', s)
#             s = re.sub(r'(\d+)/y', r'\1*v', s)  # הופך למשל 3/y ל-3*v
#             s = re.sub(r'1/y', r'v', s)
#             return s
#
#         try:
#             u_eq1_str = transform_string(self.eq1_str)
#             u_eq2_str = transform_string(self.eq2_str)
#
#             self.steps.append(f"המערכת החדשה היא:")
#             # הצגת המשוואות ב-Latex נקי
#             lhs1_uv, rhs1_uv = u_eq1_str.split('=')
#             lhs2_uv, rhs2_uv = u_eq2_str.split('=')
#
#             expr1_uv = sp.parse_expr(lhs1_uv) - sp.parse_expr(rhs1_uv)
#             expr2_uv = sp.parse_expr(lhs2_uv) - sp.parse_expr(rhs2_uv)
#
#             self.steps.append(f"1) ${sp.latex(sp.parse_expr(lhs1_uv))} = {sp.latex(sp.parse_expr(rhs1_uv))}$")
#             self.steps.append(f"2) ${sp.latex(sp.parse_expr(lhs2_uv))} = {sp.latex(sp.parse_expr(rhs2_uv))}$")
#
#             # 2. פתרון המערכת עבור u ו-v
#             sol = sp.solve([expr1_uv, expr2_uv], [u, v])
#
#             if not sol:
#                 return "Error", ["לא נמצא פתרון למערכת משתני העזר."]
#
#             val_u = sol[u]
#             val_v = sol[v]
#
#             self.steps.append("### שלב 2: פתרון עבור $u$ ו-$v$")
#             self.steps.append(f"נפתור את המערכת הליניארית ונקבל: $u = {sp.latex(val_u)}$, $v = {sp.latex(val_v)}$")
#
#             # הגנה מפני חלוקה באפס
#             if val_u == 0 or val_v == 0:
#                 self.steps.append("⚠️ אחד מערכי העזר הוא 0. לא ניתן לבצע הצבה חוזרת של $1/0$. אין פתרון למערכת.")
#                 return "אין פתרון", self.steps
#
#             # 3. חזרה למשתנים המקוריים
#             self.steps.append("### שלב 3: חזרה למשתנים המקוריים ($x, y$)")
#
#             res_x = sp.simplify(1 / val_u)
#             res_y = sp.simplify(1 / val_v)
#
#             self.steps.append(f"נבצע הצבה חוזרת:")
...
#             return "Error", [f"שגיאה בתהליך ההצבה הרציונלית: {str(e)}"]
#
#
# # פונקציית Gate עבור ה-main.py
...
#         return solver.solve()
#     except Exception as e:
#         return "Error", [f"שגיאה בניתוב: {str(e)}"]
...
class SubstitutionSystemSolver:
    ...
    def solve(self):
        ...
        self.steps.append("🚀 זיהיתי שהמשתנים נמצאים במכנה. נשתמש בשיטת **הצבת משתני עזר**.")
        ...
        try:
            ...
            if val_u == 0 or val_v == 0:
                self.steps.append("⚠️ אחד מערכי העזר יצא 0. לא ניתן לחלק ב-0, לכן אין פתרון למערכת המקורית.")
                return "אין פתרון", self.steps
...
def solve_rational_substitution_steps(eq_text):
    """השער הראשי למערכת - מחזיר מילון לעבודה חלקה עם שאר המערכת"""
    try:
        ...
        return {"result": "Error", "steps": [f"שגיאה בניתוב המערכת: {str(e)}"]}
