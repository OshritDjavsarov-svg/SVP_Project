...
# import re
#
# # 1. ייבוא מהקובץ linear_solver.py
# from solvers.linear_solver import MathRules, UniversalStepSolver
#
# # 2. ייבוא המנהלים האחרים
...
#         """מפשט משוואה אחת ומציג את הדרך צעד אחר צעד"""
#         self.steps.append(f"--- שלב א: פישוט משוואה {name} ---")
#
#         # השתמשי ב-try כדי למנוע קריסה בפירוק ביטויים לא תקינים
#         try:
#             solver = UniversalStepSolver(eq_str)
#         except Exception as e:
#             self.steps.append(f"שגיאה בפירוק המשוואה: {str(e)}")
#             return eq_str
#
#         # שלב המכנה המשותף
#         lcm = solver.get_common_denominator()
#         if lcm > 1:
#             self.steps.append(f"נכפיל במכנה המשותף {lcm}:")
#             solver.lhs_list = [sp.Mul(lcm, t, evaluate=False) for t in solver.lhs_list]
#             solver.rhs_list = [sp.Mul(lcm, t, evaluate=False) for t in solver.rhs_list]
#             self.steps.append(solver._render())
#
#             solver.lhs_list = [sp.simplify(t) for t in solver.lhs_list]
#             solver.rhs_list = [sp.simplify(t) for t in solver.rhs_list]
...
#         expanded = False
#
#         # לוגיקה משופרת לפתיחת סוגריים
#         for t in solver.lhs_list:
#             expanded_t = sp.expand(t)
#             if expanded_t != t:
#                 new_lhs.extend(MathRules.get_terms(expanded_t))
#                 expanded = True
#             else:
#                 new_lhs.append(t)
#
#         for t in solver.rhs_list:
#             expanded_t = sp.expand(t)
#             if expanded_t != t:
#                 new_rhs.extend(MathRules.get_terms(expanded_t))
#                 expanded = True
#             else:
#                 new_rhs.append(t)
#
#         if expanded:
#             solver.lhs_list, solver.rhs_list = new_lhs, new_rhs
#             self.steps.append(f"נפתח סוגריים לפי חוק הפילוג:")
#             self.steps.append(solver._render())
#
#         # סידור סופי לצורה AX + BY = C
#         # מחברים את כל האיברים ומעבירים אגפים
#         full_expr = sp.simplify(sp.Add(*solver.lhs_list) - sp.Add(*solver.rhs_list))
#
#         # חילוץ המקדם החופשי
#         constant = full_expr.subs({self.var_x: 0, self.var_y: 0})
#         clean_lhs = sp.simplify(full_expr - constant)
#         clean_rhs = sp.simplify(-constant)
#
#         self.steps.append(f"נכנס איברים דומים ונסדר את המשוואה:")
#         self.steps.append(f"$${sp.latex(clean_lhs)} = {sp.latex(clean_rhs)}$$")
#
#         return f"{clean_lhs} = {clean_rhs}"
#
#     def solve(self):
#         self.steps.append("🚀 מתחילים טיפול במערכת משוואות מורכבת")
#
#         # פישוט המשוואות
#         eq1_simple = self.simplify_side_task(self.eq1_raw, "I")
#         eq2_simple = self.simplify_side_task(self.eq2_raw, "II")
#
#         # יצירת אובייקט אלמינציה זמני לבדיקת מקדמים
#         try:
#             test_elim = EliminationSolver(eq1_simple, eq2_simple)
#             coeffs = [test_elim.a1, test_elim.b1, test_elim.a2, test_elim.b2]
#             has_unit_coeff = any(abs(c) == 1 for c in coeffs)
#         except:
#             # אם האלמינציה נכשלת בזיהוי (למשל משוואה לא ליניארית), נלך להצבה כברירת מחדל
#             has_unit_coeff = True
#
#         if has_unit_coeff:
#             self.steps.append("💡 זיהיתי משתנה עם מקדם 1. נעבור למנהל ההצבה (Substitution).")
#             final_solver = SystemSubstitutionSolver(eq1_simple, eq2_simple)
#         else:
#             self.steps.append("💡 המקדמים מורכבים. נעבור למנהל האלמינציה (Elimination) למניעת שברים.")
#             final_solver = test_elim
#
#         # הרצת הפתרון הסופי
#         result, solver_steps = final_solver.solve()
#         self.steps.extend(solver_steps)
#
...
#         return solver.solve()
#     except Exception as e:
#         return "Error", [f"שגיאה במערכת המורכבת: {str(e)}"]
