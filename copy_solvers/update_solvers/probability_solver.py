...
#
#         # 1. שליפת n ו-k מתוך C(n,k) או (n over k)
#         c_match = re.search(r'[Cc]\((\d+),(\d+)\)', clean_eq)
#         if c_match:
#             self.n = int(c_match.group(1))
#             self.k = int(c_match.group(2))
#         else:
#             # ברירת מחדל אם לא נמצא
#             self.n, self.k = 5, 3
#
#             # 2. שליפת ההסתברויות p ו-q (מספרים עשרוניים בתוך סוגריים)
#         probs = re.findall(r'\((0\.\d+)\)', clean_eq)
#         if len(probs) >= 2:
#             self.p = float(probs[0])
#             self.q = float(probs[1])
#         else:
#             # אם נמצא רק p אחד, נחשב את q כמשלים ל-1
#             if len(probs) == 1:
#                 self.p = float(probs[0])
#                 self.q = round(1 - self.p, 4)
#             else:
#                 self.p, self.q = 0.5, 0.5
#
#     def solve(self):
#         self.steps.append("--- פתרון בעיית הסתברות (נוסחת ברנולי) ---")
#
#         # הצגת המשוואה בפורמט LaTeX
#         display_eq = f"P(k={self.k}) = \\binom{{{self.n}}}{{{self.k}}} \\cdot {self.p}^{{{self.k}}} \\cdot {self.q}^{{{self.n - self.k}}}"
#         self.steps.append(f"נפתור את המשוואה: $${display_eq}$$")
#
#         # שלב 1: המקדם הבינומי
...
#         p_val = self.p ** self.k
#         q_val = self.q ** (self.n - self.k)
#
#         self.steps.append(f"1. הסתברות להצלחות: $${self.p}^{{{self.k}}} \\approx {round(p_val, 6)}$$")
#         self.steps.append(f"2. הסתברות לכישלונות: $${self.q}^{{{self.n - self.k}}} \\approx {round(q_val, 6)}$$")
#
#         # שלב 3: תוצאה סופית
#         final_result = combinations * p_val * q_val
#         self.steps.append("### שלב 3: שילוב סופי")
#         self.steps.append(f"נכפיל את כל הגורמים יחד:")
#         self.steps.append(f"$$P = {combinations} \\cdot {round(p_val, 6)} \\cdot {round(q_val, 6)}$$")
#
#         res_rounded = round(final_result, 5)
#         percent = round(final_result * 100, 2)
#
#         self.steps.append(f"**התוצאה הסופית: $$P = {res_rounded}$$**")
#         self.steps.append(f"במילים אחרות: ישנו סיכוי של **{percent}%**.")
#
#         return res_rounded, self.steps
#
#
