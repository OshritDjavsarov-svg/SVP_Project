...
class SystemSubstitutionSolver:
    ...
    def solve(self):
        ...
        iso_expr = MathUtils.clean_floats(iso_expr)
        if coeff != 1 and coeff != 0:
            self._add_step(
                 f"נחלק במקדם של {sp.latex(iso_var)} ונקבל: $${sp.latex(iso_var)} = {sp.latex(MathUtils.clean_floats(iso_expr))}$$")
                # f"נחלק במקדם של ${sp.latex(iso_var)}$ (שהוא ${sp.latex(MathUtils.clean_floats(coeff))}$$) ונקבל: ${sp.latex(iso_var)} = {sp.latex(MathUtils.clean_floats(iso_expr))}$$")
        ...
        self._add_step("נפתח סוגריים בעזרת חוק הפילוג:")
        ...
        for s in sols:
            for s in sols:
                try:
                    val_other = MathUtils.clean_floats(sp.sympify(s))

                    # 1. יצירת ביטוי הצבה ויזואלי (ללא חישוב מיידי) כדי להציג את השלב בצורה ברורה
                    visual_substitution = iso_expr.subs(other_var, sp.UnevaluatedExpr(val_other))

                    # 2. חישוב הערך הסופי בפועל לאחר כינוס וצמצום
                    ...
                    # מציגים את שלב ההצבה הויזואלי (למשל: x = 20 + 10)
                    self._add_step(f"$${sp.latex(iso_var)} = {sp.latex(MathUtils.clean_floats(visual_substitution))}$$")

                    # הוספת שלב ביניים מובנה של כינוס וחישוב
                    self._add_step("נכנס איברים דומים ונחשב את התוצאה:")

                    # מציגים את התוצאה הסופית המחושבת (למשל: x = 30)
                    self._add_step(f"$${sp.latex(iso_var)} = {sp.latex(val_iso)}$$")

                    # זיהוי דינמי מי מהם הוא x ומי מהם הוא y בהתאם למשתנה שבודד
                    x_val = val_other if other_var == self.var1 else val_iso
                    y_val = val_iso if other_var == self.var1 else val_other

                    final_solutions.append(f"x = {sp.latex(x_val)} \\quad , \\quad y = {sp.latex(y_val)}")
                except:
                    continue

        # אם יש יותר מפתרון אחד (במקרה של משוואה ריבועית), נחבר אותם עם מילת קישור
        res_str = " \\\\quad \\text{או} \\\\quad ".join(final_solutions)
