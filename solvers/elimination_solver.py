...
class EliminationSolver:
    ...
    def solve(self):
        ...
        # הפיכת הערכים ל-LaTeX
        x_latex = sp.latex(res_x)
        y_latex = sp.latex(res_y)

        # בניית הפורמט המבוקש עם המשתנים x ו-y ורווחים מתאימים
        final_ans = f"x = {x_latex} \\quad , \\quad y = {y_latex}"

        self.steps.append("---")
        # הצגת הפתרון הסופי בצורה ממורכזת ובולטת (עם $$)
        self.steps.append(f"**הפתרון הסופי:** $_{final_ans}_")
...
if __name__ == "__main__":
    ...
    run_test("מערכת רגילה", "x+y=30,2x+4y=100")
